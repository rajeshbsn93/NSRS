...
@Component({
  selector: 'app-kicMonitoringTrainingDetails',
  templateUrl: './kicMonitoringTrainingDetails.component.html',
  styleUrls: ['./kicMonitoringTrainingDetails.component.css'],
  standalone:true,
...
export class KicMonitoringTrainingDetailsComponent implements OnInit {
  ...
  AddMultiTagArray(modal:any):FormGroup{
    return this.fb.group({
      ...
      academy_kuid:[modal.action == 'edit' ? (this.userDetails.role_id == 82 ? modal.data.academy_KUID : modal.data.kiC_KUID) : this.userDetails.nsrs_id,Validators.required],
      ...
      training_Video_Link:[ {value:modal.action == 'edit' ? this.KicUsersRoleId.kicAdmin == this.userDetails.role_id ?  modal.data.training_Video_Link : modal.data.upload_doc_path :'',disabled: modal.data.is_trainging_provided ? false : true},[Validators.required]],

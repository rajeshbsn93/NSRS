...
import { CoachEditDetailsWeedoutComponent } from 'src/app/role-inner-pages/modal-window/coach-EditDetails-Weedout/coach-EditDetails-Weedout.component';
import { AddCampCoachComponent } from '../modal-window/add-camp-coach/add-camp-coach.component';
...
@Component({
  selector: 'app-camp-coach',
  templateUrl: './camp-coach.component.html',
  styleUrls: ['./camp-coach.component.css'],
  standalone:true,
  imports:[CommonModule,MaterialModule,LoaderComponent,ReactiveFormsModule,FormsModule],
  providers: [DatePipe]
})
export class CampCoachComponent implements OnInit {
  displayedColumns: string[] = ['kitd_unique_id', 'full_name','designation','date_of_joining','mobile_number','state_name','is_Insured'];
  dataSource:any
  ...
  getCampOfficialDetail(){
    this.mainListLoader=true
    this.campInnerPagesService.campOfficialDetail(this.userDetails.user_id,2).pipe(first(),map((items:any)=>{
  ...
  openmultiTag(){
    if(this.searchFilter.value.disciplineToSearch == '') this.alertService.swalPopWarning('Please Select Discipline')
    else{
      const modalRef = this.modalService.open(AddCampCoachComponent,{size:'xl', centered:true})

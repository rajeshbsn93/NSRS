...
@Component({
  ...
  standalone:true,
  imports: [CommonModule, MaterialModule, LoaderComponent, ReactiveFormsModule, DocumentNotFoundComponent],
  providers:[
...
export class DocumentViewManageAthleteComponent implements OnInit {
  ...
  rejectButton(){
    this.esignForm.controls['remark'].setValidators([Validators.required])
    this.esignForm.controls['remark'].updateValueAndValidity();
    if(this.esignForm.valid){      
      ...
      this.savePlayerDobverification(data)
  ...
  verifyButton(){
    this.esignForm.controls['remark'].setValidators(null)
    this.esignForm.controls['remark'].updateValueAndValidity();
    const data = {
      player_detail_id: this.elementRowData.player_detail_id,
      date_of_birth: this.elementRowData.date_of_birth,

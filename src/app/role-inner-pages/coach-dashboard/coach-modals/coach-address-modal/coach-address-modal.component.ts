...
@Component({
  selector: 'app-coach-address-modal',
  templateUrl: './coach-address-modal.component.html',
  styleUrls: ['./coach-address-modal.component.css'],
  standalone: true,
  imports: [CommonModule, MaterialModule, ReactiveFormsModule, NgbTooltipModule, LoaderComponent],
})
export class CoachAddressModalComponent implements OnInit {
  ...
  getOfficialAddressInfo() {
    ...
    this.coachDashboardService
      .getOfficialAddressInfo(this.userDetails.user_id)
      .subscribe({
        next: (response: any) => {
          this.coachAddressInfoData = response;
  ...
  getDistrictCommunication(stateId: number) {
    if (!this.communicationAddressForm.dirty)
      this.communicationAddressForm.controls['comm_add_district_id'].setValue(
        Number(this.coachAddressInfoData.comm_add_district_id)
      );
    ...
    return this.commonSharableService.districtMasterList(stateId).subscribe({
      next: (response) => {
        ...
        if (!this.communicationAddressForm.dirty) {
          this.getBlocktCommunication(Number(this.coachAddressInfoData.comm_add_district_id));
          this.getPinCodeCommunication(Number(this.coachAddressInfoData.comm_add_district_id));
        }
      },
      error: (err) => {
        console.error(err);
  ...
  getCityCommunication(stateId: number) {
    if (!this.communicationAddressForm.dirty)
      this.communicationAddressForm.controls['comm_city_id'].setValue(
        Number(this.coachAddressInfoData.comm_city_id)
      );
  ...
  getBlocktCommunication(districtId: number) {
    if (!this.communicationAddressForm.dirty)
      this.communicationAddressForm.controls['comm_add_block_id'].setValue(
        Number(this.coachAddressInfoData.comm_add_block_id)
      );
  ...
  getPinCodeCommunication(districtId: number) {
    if (!this.communicationAddressForm.dirty)
      this.communicationAddressForm.controls['comm_pincode'].setValue(this.coachAddressInfoData.comm_pincode);
      ...
      next: (response) => {
        ...
        if (this.communicationAddressForm.controls['perm_comm_add_same'].value) {
          ...
          if (!this.communicationAddressForm.get('perm_state_id')?.dirty)
            this.communicationAddressForm.controls[
              'perm_add_district_id'
            ].setValue(Number(this.coachAddressInfoData.perm_add_district_id));
    ...
    return this.commonSharableService.cityMasterList(stateId).subscribe({
      next: (response) => {
        ...
        if (this.communicationAddressForm.controls['perm_comm_add_same'].value) {
          ...
          this.communicationAddressForm.controls['perm_city_id'].setValue(
            Number(this.coachAddressInfoData.perm_city_id)
          );
        }
      },
      error: (err) => {
      ...
      next: (response) => {
        ...
        if (this.communicationAddressForm.controls['perm_comm_add_same'].value) {
          this.communicationAddressForm.controls['perm_add_block_id'].setValue(
            Number(this.communicationAddressForm.controls['comm_add_block_id'].value)
          );
        } else if (!this.communicationAddressForm.dirty) {
          this.communicationAddressForm.controls['perm_add_block_id'].setValue(
            Number(this.coachAddressInfoData.perm_add_block_id)
          );
        }
      },
      error: (err) => {
    ...
    return this.commonSharableService.pincodeMasterList(districtId).pipe(map((res: any) => {
      ...
      next: (response) => {
        ...
        if (this.communicationAddressForm.controls['perm_comm_add_same'].value) {
          ...
          this.communicationAddressForm.controls['perm_pincode'].setValue(this.coachAddressInfoData.perm_pincode);
        }
      },
      error: (err) => {
        console.error(err);
  ...
  save() {
    ...
    this.loader = true;
    this.coachDashboardService.saveOfficialAddressInfo(
      {...this.communicationAddressForm.getRawValue(), official_detail_id: this.userDetails.user_id}
    ).subscribe({
      next: (response) => {
        this.loader = false;
        if (response) {

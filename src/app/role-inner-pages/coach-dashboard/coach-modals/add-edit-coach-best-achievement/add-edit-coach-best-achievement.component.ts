...
@Component({
    selector:'app-add-edit-coach-best-achievement',
    templateUrl:'./add-edit-coach-best-achievement.component.html',
    styleUrls:['./add-edit-coach-best-achievement.component.css'],
    standalone:true,
    imports:[CommonModule,MaterialModule,LoaderComponent,ReactiveFormsModule,YearFormatDirective,NgbTooltipModule],
    providers: [
...
export class AddEditCoachBestAchievementComponent implements OnInit{
 ...
 @ViewChild('document_pathInput') document_pathInput!: ElementRef<any>;
    ...
    ngOnInit(): void {
        if(this.editRowData !=(null)) {
            this.achievement_detail_id = this.editRowData?.coach_achievement_detail_id;
            this.player_detail_id = this.editRowData?.official_detail_id
    ...
    handleYearSelected(event:Moment,yearOpen: MatDatepicker<Moment>) {
        ...
        this.tournamentService.GetEventdetailSportwise(sport_id).subscribe({
          next:(response:any)=>{
            ...
            if(response.length > 0){
                  let profileData= JSON.parse(JSON.stringify(JSON.parse(localStorage.getItem('sessiondata')!)?.profileData))
                       if(profileData?.gender==='Male'){
                        profileData.genderId=1
                       }else{
                        profileData.genderId=2
                       }
                       const filterSportData = response.filter((item:any)=>{
                        if((item.gender_id === profileData?.genderId || item.gender_id === 3)){
                          return item
                        }
                      })
      ...
      changeTournament(event:any){
        ...
        this.tournamentService.eventdetailTournamnetwise(tournamentId).subscribe({
          next:(response:any)=>{
            this.loader = false;
            let reqProfileData= JSON.parse(JSON.stringify(this.userSesionData?.profileData))
            if(reqProfileData?.gender==='Male'){
              reqProfileData.genderId=1
            }else{
              reqProfileData.genderId=2
            }
            const userDiscipline = Number(this.userSesionData?.profileData?.sport_id)
            const filterSportData = response.filter((item:any)=>{
              if((item.sport_id === userDiscipline) && (item?.gender_id===reqProfileData.genderId || item?.gender_id===3 )){
                return item
              }
            })
      ...
      fileUpload(event: any) {
        ...
        if (extFile == "jpg" || extFile == "jpeg" || extFile == "png" || extFile == "pdf") {
          ...
          formData.append("path", "Coach\\Achievement");
      ...
      save(){
        ...
        const player_detail_id = this.storageService.getUserDetails().user_id;
        let competitionName = '' ;
        let id=0; 
        if(this.achievementForm.valid && this.document_pathUrl !=null){  
          ...
          this.loader = true               
          this.coachAchievementService.SaveCoachAchieveMent(
      ...
      //       }
            ...
            fileSave(){
              ...
              this.loader = true
                this.coachAchievementService.SaveCoachAchieveMent(
                  this.achievement_detail_id,player_detail_id,event_id,represented,position,result,this.document_pathUrl,tournament_id,

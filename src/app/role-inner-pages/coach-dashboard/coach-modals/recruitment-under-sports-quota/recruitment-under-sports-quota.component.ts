...
import { AddEditRecruitmentSportsQuotaComponent } from "../add-edit-recruitment-sports-quota/add-edit-recruitment-sports-quota.component";

@Component({
  selector: 'app-recruitment-under-sports-quota',
  templateUrl: './recruitment-under-sports-quota.component.html',
  styleUrls: ['./recruitment-under-sports-quota.component.css'],
  standalone: true,
...
export class RecruitmentUnderSportsQuotaComponent implements OnInit {
  ...
  addEdit(data: any) {
    const modalRef = this.modalService.open(AddEditRecruitmentSportsQuotaComponent, { centered: true, size: 'xl', backdrop: 'static', keyboard: false });
  ...
  navigateToAchievement() {
    if (this.userDetails.role_id === 2) {
      this._routs.navigate(['coach-achievement']);
    } else if (this.userDetails.role_id === 103) {
      this._routs.navigate(['sport-scientist-achievement']);
    }
  }

...
import { AddEditTrainingWorkshopComponent } from "../add-edit-training-workshop/add-edit-training-workshop.component";

@Component({
    selector:'app-training-workshop',
    templateUrl:'./training-workshop.component.html',
    styleUrls:['./training-workshop.component.css'],
    standalone:true,
...
export class TrainingWorkshopComponent implements OnInit{
    ...
    experienceTableColumns:string[] = ['sl','type','level','name','date','action'];
    ...
    get_training_workshop_conference_details(){
        this.loader = true;
        this.experienceService.training_workshop_conference_details(this.userDetails.user_id).subscribe({
            next:(response:any)=>{
                this.loader = false;
    ...
    confirmDelete(){
        this.loader = true;
        this.experienceService.DeleteTrainingWorkshopConferenceDetails(this.deleteRowData.training_workshop_conference_detail_id).subscribe({
            next:(response)=>{
                this.loader = false;
                if(response){
                    this.get_training_workshop_conference_details();
    ...
    addEdit(data:any){
        const modalRef = this.modalService.open(AddEditTrainingWorkshopComponent,{centered:true,size:'xl',backdrop:'static',keyboard:false});

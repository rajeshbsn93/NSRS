...
import { AddEditMembershipScientificComponent } from "../add-edit-membership-scientific/add-edit-membership-scientific.component";

@Component({
    selector:'app-membership-scientific',
    templateUrl:'./membership-scientific.component.html',
    styleUrls:['./membership-scientific.component.css'],
    standalone:true,
...
export class MembershipScientificComponent implements OnInit{
    ...
    experienceTableColumns:string[] = ['sl','body_type','body_name','body_year','remark','action'];
    ...
    get_member_scientific_bodies_details(){
        this.loader = true;
        this.experienceService.member_scientific_bodies_details(this.userDetails.user_id).subscribe({
            next:(response:any)=>{
                this.loader = false;
    ...
    confirmDelete(){
        this.loader = true;
        this.experienceService.deleteMemberScientificBodiesDetails(this.deleteRowData.id).subscribe({
            next:(response)=>{
                this.loader = false;
                if(response){
                    this.get_member_scientific_bodies_details();
    ...
    addEdit(data:any){
        const modalRef = this.modalService.open(AddEditMembershipScientificComponent,{centered:true,size:'xl',backdrop:'static',keyboard:false});

...
import { AthleteEducationEntity, AthleteEducationInfoService, BoardEntity, EDUCATION, EditAthleteEducationEntity, INSTT_TYPE, MediumEntity, PASSING_STATUS, UniversityEntity } from 'src/app/_common/services/role-inner-pages-services/athlete-services/athlete-education-info.service';
...
@Component({
  standalone: true,
  selector: 'app-athlete-add-education-info',
  templateUrl: './athlete-add-education-info.component.html',
  styleUrls: ['./athlete-add-education-info.component.css'],
...
export class AthleteAddEducationInfoComponent implements OnInit, OnDestroy {
  ...
  editEducation: AthleteEducationEntity | null = null;
  ...
  getBoardList() {
    this.athleteEducationInfoService.getBoardList().subscribe(
      (response: BoardEntity[]) => this.boardList = response
    );
  }

  getMediumList() {
    this.athleteEducationInfoService.getMediumList().subscribe(
  ...
  saveDetails() {
    ...
    const payload = {
      player_detail_id: this.storageService.getUserDetails()?.user_id,
    ...
    this.athleteEducationInfoService.saveEducationInfo(payload).subscribe({
      next: (response: boolean) => {
        if (response) {
          this.alertService.swalPopSuccess('Athlete education details inserted successfully!');
  ...
  updateDetails() {
    ...
    const payload: EditAthleteEducationEntity = {
      player_education_id: this.editEducation!.player_education_id,
    ...
    this.athleteEducationInfoService.editEducationInfo(payload).pipe(first()).subscribe({
      next: (response: boolean) => {
        this.loader = false;
        if (response) {
          this.activeModal.close(true);

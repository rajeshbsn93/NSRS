...
@Component({
  selector: 'app-athlete-add-bank-info',
  standalone: true,
  imports: [CommonModule, MaterialModule, ReactiveFormsModule],
  templateUrl: './athlete-add-bank-info.component.html',
  styleUrls: ['./athlete-add-bank-info.component.css']
})
export class AthleteAddBankInfoComponent implements OnInit {
  ...
  basicDetailsReactiveForm() {
    this.bankForm = this.formBuilder.group({
      id: [0],
      player_id: [this.userDetails.user_id || 0],
  ...
  setFormValues() {
    ...
    this.bankInformationService.athletBankInfo(this.userDetails.user_id, 'M').subscribe({
      next: (response: any) => {
        ...
        if(response.length ==0) this.checkprimary = true;
        this.bankForm.controls['bank_branch'].disable()
  ...
  changeIfsc(event: any) {
    if (event != '') {
      this.loader = true;
      this.bankInformationService.bankDetail(event).subscribe({
        next: (response) => {
          this.loader = false;
          console.log(response)
          if (response != null) {
            this.bankForm.controls['bank_name'].setValue(response.bank_name);
            this.bankForm.controls['bank_branch'].setValue(response.bank_branch)
            this.bankForm.controls['bank_address'].setValue(response.bank_address)
            this.bankForm.controls['bank_branch'].disable()
            this.bankForm.controls['bank_address'].disable();
            this.bankForm.controls['bank_name'].disable();
          } else {
            ...
            this.bankForm.controls['bank_branch'].reset();
            this.bankForm.controls['bank_address'].reset();
            this.bankForm.controls['bank_account_number'].reset();
          }
        },
        error: (err) => {
          this.loader = false;
  ...
  save() {
    if (this.bankForm.valid) {
      ...
      this.bankForm.value.player_id = this.userDetails.user_id
      if (this.bankForm.value.bank_account_number != null) {
        this.bankForm.value.bank_account_number = this.bankAccountInput.nativeElement.value
      } else {
        this.bankForm.value.bank_account_number = ''
      }
      if (this.bankFileUploadRes != undefined) {
        this.bankForm.value.cancelled_cheque_upload = this.bankFileUploadRes.filedataList[0].filePath;
      } else {
        if (this.bankFileUploadUrl != null) {
          this.bankForm.value.cancelled_cheque_upload = this.bankFileUploadUrl
        }
      }
      ...
      this.bankInformationService.saveAthletBankInfo(formData).subscribe({
        next: (response) => {
          if (response) {
            this.loader = false
            this.alertService.swalPopSuccess('Bank Account Updated Successfully!');

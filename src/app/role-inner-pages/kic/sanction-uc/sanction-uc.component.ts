...
@Component({
  selector: 'app-sanction-uc',
  templateUrl: './sanction-uc.component.html',
  styleUrls: ['./sanction-uc.component.css'],
})
export class SanctionUcComponent implements OnInit {
  users: any[] = [];
  ...
  getSanctionUcList() {
    ...
    this.sanctionService.getSanctionUcList(this.userDetails.user_id, this.userDetails.role_id, this.moduleType).subscribe({
      next: (res: any) => {
  ...
  openReleasedAmountPopup(value: any) {
    this.commentHistoryTableModalRef = this._modalService.open(SanctionReleasedAmountUcComponent, { size: 'lg', centered: true, backdrop: 'static' });

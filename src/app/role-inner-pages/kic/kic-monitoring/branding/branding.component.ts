...
import { BrandingModalComponent } from 'src/app/standalone_components/modal-window/kicMonitoringModalWindows/brandingModal/brandingModal.component';
...
export class BrandingComponent implements OnInit {
  ...
  // displayedRcColumns:string[]=['sno','stateName','kiC_KUID','kiC_NAME','brandingAtKIC', 'uploadImage', 'brandingRemarks','action'];
  ...
  getBrandingList(check?:any){
    ...
    this._equipmentService.getBranding(this.userDetails.user_id,this.userDetails.role_id).pipe(takeUntil(this.unsubscribe)).subscribe({
      next:(res:any)=>{
  ...
  rdChange(elementData:any,isStateChange?:any){
    ...
    this.mainLoader=true
    this._equipmentService.getBranding(elementData.user_id,elementData.role_id).subscribe({
      next:(res:any)=>{
        this.mainLoader=false
        this.rdListshow=true
        this.stateName=res.statewisedata[0].state

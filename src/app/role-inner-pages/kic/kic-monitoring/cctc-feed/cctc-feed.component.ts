...
import { KicMonitoringCctvFeedComponent } from 'src/app/standalone_components/modal-window/kicMonitoringModalWindows/kicMonitoringCctvFeed/kicMonitoringCctvFeed.component';
...
@Component({
  selector: 'app-cctc-feed',
  templateUrl: './cctc-feed.component.html',
  styleUrls: ['./cctc-feed.component.css']
})
export class CctcFeedComponent implements OnInit {
  ...
  getCctvFeedData(){
    ...
    this._cctvFeedService.getCctvFeedList(this.userDetails.user_id,this.userDetails.role_id).subscribe({
      next:(res:any)=>{
  ...
  openAddCctvModal(action:string,index:number){
    const modelRef = this._modalService.open(KicMonitoringCctvFeedComponent,{size:'xl',centered:true, backdrop: 'static'})

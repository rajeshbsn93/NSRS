...
@Component({
  selector: 'app-sanction-released-amount-uc',
  templateUrl: './sanction-released-amount-uc.component.html',
  styleUrls: ['./sanction-released-amount-uc.component.css'],
  providers: [
...
export class SanctionReleasedAmountUcComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.sanctionReleaseForm.valid && this.calculateTotalUCAmount() <= this.released_Amount) {
      let payload = {
        ...
        sanction_Id: this.sanction_Id,
        scheme_Role_Id: this.userkicAndKisceRole.kicAdmin
      }
  ...
  openPdfFormat() {
    ...
    link.href = url;
    link.download = 'text.pdf'; // Optional: specify the filename for the download

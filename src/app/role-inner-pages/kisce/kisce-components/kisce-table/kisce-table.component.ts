...
@Component({
  selector: 'app-kisce-table',
  templateUrl: './kisce-table.component.html',
  styleUrls: ['./kisce-table.component.css']
})
export class KisceTableComponent implements OnInit {
...
displayedColumns: string[] = ['sno', 'duration', 'male_count', 'female_count', 'total_count', 'upload_date', 'from_date', 'to_date', 'recordFile'];
  ...
  getAttendance() {
    ...
    this._kicAttendanceService.getKicAttendance(payload,80).subscribe(
      {
        next: (res: any) => {

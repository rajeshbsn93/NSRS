...
import { KisceRcTableComponent } from '../kisce-rc-table/kisce-rc-table.component';
import { FormBuilder, FormGroup } from '@angular/forms';
import { CommonSharableService } from 'src/app/_common/services/common-services/commonSharable.service';
@Component({
  selector: 'app-kisce-ho-table',
  templateUrl: './kisce-ho-table.component.html',
  styleUrls: ['./kisce-ho-table.component.css']
})
export class KisceHoTableComponent implements OnInit {
 ...
 displayedColumns: string[] = ['sno', 'rcName', 'stateName', 'totalDistrict', 'totalAthlete'];
  ...
  getKicAttendanceHoDetails(userId: number, roleId: number) {
    this._kicAttendanceService.getKicAttendanceHo(userId, roleId, 80).subscribe(
      {
        next: (res: any) => {
  ...
  openKicRcDetailsPopup(item: any) {
    this.kicRcTableModalRef = this._modalService.open(KisceRcTableComponent, { size: 'xl', centered: true, backdrop: 'static' });

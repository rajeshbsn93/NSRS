...
import { KisceTableComponent } from '../kisce-table/kisce-table.component';
...
@Component({
  selector: 'app-kisce-rc-table',
  templateUrl: './kisce-rc-table.component.html',
  styleUrls: ['./kisce-rc-table.component.css']
})
export class KisceRcTableComponent implements OnInit {
...
displayedColumns: string[] = ['sno', 'stateName', 'districtName', 'kiud', 'totalAthlete', 'recordFile'];
  ...
  getKicAttendanceRcDetails(userId: number, roleId: number) {
    this._kicAttendanceService.getKicAttendanceRc(userId, roleId,80).subscribe(
      {
        next: (res: any) => {
  ...
  openKicDetailsPopup(item: any) {
    this.kicTableModalRef = this._modalService.open(KisceTableComponent, { size: 'xl', centered: true, backdrop: 'static' });
  ...
  getKicAttendanceByKicRcId(kicRcId: any) {
    ...
    this._kicAttendanceService.getKicAttendanceByKicRcId(payload, 80).subscribe(
      {
        next: (res: any) => {
          this.isLoading = false;
          if (res?.status === 'success') {

...
@Component({
  selector: 'app-kisce-state-table',
  templateUrl: './kisce-state-table.component.html',
  styleUrls: ['./kisce-state-table.component.css']
})
export class KisceStateTableComponent implements OnInit {
 ...
 displayedColumns: string[] = ['sno', 'kiud', 'attendanceRecord', 'maleCount', 'femaleCount', 'total', 'uploadDate', 'fromDate', 'toDate', 'recordFile'];
  ...
  getKicAttendanceStateDetails(userId: number, roleId: number) {
    this._kicAttendanceService.getKicAttendanceState(userId, roleId,80).subscribe(
      {
        next: (res: any) => {

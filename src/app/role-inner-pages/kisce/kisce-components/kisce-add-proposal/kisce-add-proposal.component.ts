...
import { KisceSanctionListComponent } from '../../kisce-sanction-modal-view/kisce-sanction-list/kisce-sanction-list.component';


@Component({
  selector: 'app-kisce-add-proposal',
  templateUrl: './kisce-add-proposal.component.html',
  styleUrls: ['./kisce-add-proposal.component.css']
})
export class KisceAddProposalComponent implements OnInit {
 ...
 financialYearMaster: FinancialYearMaster = [];
  ...
  formInitialization() {
    this.addSanctionForm = this._fb.group({
      ...
      scheme_Roll_Id: [80, [Validators.required]],
    });
  }
  ...
  const stateID = this.addSanctionForm.get('stateId')?.value
   this.kicSanctionService.getKicListbyStateId(stateID, 80).subscribe({
    next: (res: any) => {
   this.kisListData = res.data
      const kicListModalRef = this.modalService.open(KisceSanctionListComponent, { size: 'xl', centered: true });

...
import { KisceMonitoringTrainingDetailsComponent } from 'src/app/standalone_components/modal-window/kisceMonitoringModalWindows/kisce-monitoring-training-details/kisce-monitoring-training-details.component';
...
export class TrainingDetailsComponent implements OnInit {
 ...
 @ViewChild('statePaginator', { read: MatPaginator }) statePaginator!: MatPaginator
  ...
  openAddTrainingDetailsModal(action:string,element:any){
    const modelRef = this._modalService.open(KisceMonitoringTrainingDetailsComponent,{size:'xl',centered:true, backdrop: 'static'})
  ...
  getTrainingDetailsList(check?:any){
    ...
    this._trainingDetailsService.getTrainingDetailsList(this.userDetails.user_id,this.userDetails.role_id, 80).subscribe({
      next:(res:any)=>{
  ...
  rdChange(elementData:any,check?:any){
    ...
    this.mainLoader=true
    this._trainingDetailsService.getTrainingDetailsList(elementData.user_id,elementData.role_id,80).subscribe({
      next:(res:any)=>{
        this.rdListshow=true
        this.mainLoader=false
        this.stateName = res.stateWisedata[0].state;

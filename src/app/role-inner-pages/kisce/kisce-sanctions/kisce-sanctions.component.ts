...
import { CommentHistoryComponent } from '../../kic/kic-components/comment-history/comment-history.component';
import { AddProposalComponent } from '../../kic/kic-components/add-proposal/add-proposal.component';
import { SanctionReleasedAmountComponent } from '../../kic/sanction-released-amount/sanction-released-amount.component';
import { SanctionTotalAmountComponent } from '../../kic/sanction-total-amount/sanction-total-amount.component';
...
@Component({
  selector: 'app-kisce-sanctions',
  templateUrl: './kisce-sanctions.component.html',
  styleUrls: ['./kisce-sanctions.component.css'],
  providers: [
...
export class KisceSanctionsComponent implements OnInit {
  ...
  onChangeRc(rc_Id: any) {
    ...
    this.sanctionService.getSanctionStateListByRC(rc_Id).subscribe({
      next: (res: any) => {
        ...
        console.log('res1-------------', this.statelist);
      },
      error: (errors: any) => { },
    })
  }
  ...
  getProposalList() {
    ...
    this.sanctionService.getSanctionListNew(this.userDetails.user_id, this.userDetails.role_id, 80).subscribe({
      next: (res: any) => {
  ...
  openTotalAmountPopup(id: any, index: number) {
    this.commentHistoryTableModalRef = this._modalService.open(KisceSanctionTotalAmountComponent, { size: 'xl', centered: true });
    this.commentHistoryTableModalRef.componentInstance.kiC_id = id?.value?.kiC_id;
    this.commentHistoryTableModalRef.componentInstance.sanction_Id = id?.value?.sanction_Id;
    this.commentHistoryTableModalRef.componentInstance.sanction_No = id?.value?.sanction_No;
    this.commentHistoryTableModalRef.componentInstance.is_kic_kisce = 'kisce';
    this.commentHistoryTableModalRef.componentInstance.onAdd.subscribe(() => {
      this.getProposalList();
    });
  }
  ...
  addSanctionPopup() {
    this.commentHistoryTableModalRef = this._modalService.open(KisceAddProposalComponent, { size: 'xl', centered: true });
    this.commentHistoryTableModalRef.componentInstance.onAdd.subscribe(() => {
      this.getProposalList();
    });
  }

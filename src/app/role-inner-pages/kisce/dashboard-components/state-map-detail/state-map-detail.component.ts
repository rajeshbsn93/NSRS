...
export class StateMapDetailComponent implements OnInit {
      ...
      tooltip: {
        ...
        formatter: function () {
          ...
          if (point?.districtData?.length > 0) {
            tooltipContent += `<tr>`
            tooltipContent += `<td> KISCE Name: </td> <td> <b>${point?.districtData[0]?.academy_name}</b></td>`
            tooltipContent += `</tr>`
            tooltipContent += `<tr>`
            tooltipContent += `<td> NSRS ID: </td> <td> <b>${point?.districtData[0]?.kiud}</b></td>`
            tooltipContent += `</tr>`
            tooltipContent += `<tr>`
            tooltipContent += `<td> COACH Name: </td> <td> <b>${point?.districtData[0]?.pca_name != undefined && point?.districtData[0]?.pca_name != null ? point?.districtData[0]?.pca_name : '-'}</b></td>`
            tooltipContent += `</tr>`
            tooltipContent += `<tr>`
  ...
  getMapList() {
    ...
    this._kicDashboardService.indiaMapListForKisce(payload).subscribe({
      next: (res: any) => {
        if (res?.status === 1) {
  ...
  stateMapDetail() {
    ...
    this._kicDashboardService?.stateMapListforKisce(payload).subscribe(
      {
        next: (res: any) => {
          if (res?.status === 1) {
